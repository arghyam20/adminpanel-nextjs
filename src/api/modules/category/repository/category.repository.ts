...
export class CategoryRepository implements CategoryRepositoryContract {
  private get model() { return prisma.category; }
  ...
  async findById(id: number) { return this.model.findFirst({ where: { id, ...this.baseWhere } }) as never; }
  async findBySlug(slug: string) { return this.model.findFirst({ where: { slug, ...this.baseWhere } }) as never; }
  async create(data: CreateCategoryDto) { return this.model.create({ data: data as never }) as never; }
  async update(id: number, data: UpdateCategoryDto) { return this.model.update({ where: { id }, data: data as never }) as never; }
  async softDelete(id: number) { return this.model.update({ where: { id }, data: { isDeleted: true } }) as never; }
}
