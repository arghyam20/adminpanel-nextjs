...
export class FaqRepository {
  private get model() { return prisma.faq; }
  ...
  async paginate(options: QueryOptions) {
    ...
    const [items, total] = await Promise.all([
      this.model.findMany({ where, skip: (options.page - 1) * options.pageSize, take: options.pageSize, orderBy: { [options.sortBy ?? "ordering"]: options.sortOrder ?? "asc" } }),
  ...
  async create(data: CreateFaqDto) { return this.model.create({ data: data as never }); }
  async update(id: number, data: UpdateFaqDto) { return this.model.update({ where: { id }, data: data as never }); }
