...
export class ServiceRepository {
  private get model() { return prisma.service; }
  private get baseWhere() { return { isDeleted: false }; }
  private buildSearch(search?: string) {
    if (!search) return {};
    return { OR: [{ title: { contains: search } }, { shortDesc: { contains: search } }, { metaTitle: { contains: search } }] };
  }
  async paginate(options: QueryOptions) {
    ...
    const [items, total] = await Promise.all([
      this.model.findMany({ where, include: INCLUDE, skip: (options.page - 1) * options.pageSize, take: options.pageSize, orderBy: { [options.sortBy ?? "ordering"]: options.sortOrder ?? "asc" } }),
  ...
  async create(data: CreateServiceDto) { return this.model.create({ data: data as never, include: INCLUDE }); }
  async update(id: number, data: UpdateServiceDto) { return this.model.update({ where: { id }, data: data as never, include: INCLUDE }); }
